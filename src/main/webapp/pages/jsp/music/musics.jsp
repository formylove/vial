...
<div class="vol-tracklist" id="luooPlayerPlaylist">
        <div class="player-large rounded clearfix" data-index="0">
            ...
            <div class="player-ct" id="playerCt" style="display: block;">
                ...
                <p style="margin-top: 20px;" class="track-meta motto ${empty musics[0].motto?'hide':''}"><span class="PLAlbum" title="${musics[0].motto }">${musics[0].motto }</span></p>
...
<s:iterator value="musics" id="m" status="st">
		...
		<s:else>
		<audio src="${musicUrl}${m.url }"  data-motto= "${m.motto}" id="music_${st.index}" data-lyric="${m.lyric}" data-index = "${st.index}"  data-index = "${st.index}" data-name="${m.name}" data-cover="${m.bcover}" data-singer = "${m.singer.name}"></audio>
		</s:else>
            ...
            <div class="track-detail-wrapper" >                <div class="track-detail-arrow">
                    <img src="${imageUrl}img/icon/trian.png">
                </div>
