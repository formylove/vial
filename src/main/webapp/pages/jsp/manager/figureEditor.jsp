	    	...
	    	<table cellpadding="5">
	    		<tr>
	    			...
	    			<input id="domain" class="easyui-effect-input" type = "text" name="figure.domain" class="" value='${figure.domain}' list='domains'/>
      				<datalist id='domains'>
      				<s:iterator value="@ink.moshuier.silken.service.impl.FigureServiceImpl@getDomains()" id="o">
	    		...
	    		<tr>
	    			<td>宗教:</td>
	    			<td>
	    			<s:select list="@ink.moshuier.silken.service.impl.ReligionServiceImpl@getReligions()" listKey="key" listValue="value" cssClass="easyui-combobox" id="religion" name='figure.religion.id' ></s:select>
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>种族:</td>
	    			<td>
	    			<s:select list="@ink.moshuier.silken.service.impl.PeopleServiceImpl@getPeoples()" listKey="key" listValue="value" cssClass="easyui-combobox" id="echnic" name='figure.echnic.id' ></s:select>
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>出生:</td>
	    		...
	    		<tr>
	    			...
	    			<input id="job" class="easyui-effect-input" type = "text" name="figure.job" value='${figure.job}' list='jobs'/>
      				<datalist id='jobs'>
     				<s:iterator value="@ink.moshuier.silken.service.impl.FigureServiceImpl@getJobs()" id="o">
					<option value=' <s:property value='o'/>'/>
      				</s:iterator>
					</datalist>
	    			<td>
				...
				<div id="preview-pane" class="fleft margin-r-55">
					<div id ="preview-container" class="preview-container-vertical" onclick="$(file).click();">
						<img src="<s:if test='figure.cover != null && figure.cover != ""'>${imageUrl}${figure.cover}</s:if><s:else>img/common/vertical_default.jpg</s:else>" id="prevImg" class="cover-vertical jcrop-preview" alt="Preview" />
					</div>
				</div>
				</div>
				</div>
