...
<div class="easyui-panel" title="书" style="width: 760px; height: 1085px">
	<div style="padding-top: 10px;padding-left: 10px">
	<form id="book-editor" action="manager/opus/save/" method="post">
			...
			<input type="hidden" name="opus.type" id="type" value="book">
			<div class="clearfix">
			<div class="fleft">
	    	<table cellpadding="5">
	    		<tr>
	    		...
	    		<tr>
	    			<td>类型:</td>
	    			<td>
	    			<s:select list="@ink.moshuier.silken.service.impl.GenreServiceImpl@getAll().{?#this.type=='book'}" listKey="id" listValue="name" id="genre0" name="opus.genres[0].id" cssClass="easyui-combobox"></s:select>
	    			<s:select list="@ink.moshuier.silken.service.impl.GenreServiceImpl@getAll().{?#this.type=='book'}" listKey="id" listValue="name" id="genre1" name="opus.genres[1].id" cssClass="easyui-combobox"></s:select>
	    			<s:select list="@ink.moshuier.silken.service.impl.GenreServiceImpl@getAll().{?#this.type=='book'}" listKey="id" listValue="name" id="genre2" name="opus.genres[2].id" cssClass="easyui-combobox"></s:select>
				...
				<div id="preview-pane" class="fleft margin-r-55">
					<div id ="preview-container" class="preview-container-vertical"  onclick="$(file).click();">
						<img src="<s:if test='opus.cover != null && opus.cover != ""'>${imageUrl}${opus.cover}</s:if><s:else>img/common/horizontal_default.jpg</s:else>" id="prevImg" class="cover-vertical jcrop-preview" alt="Preview" />
					</div>
				</div>
				</div>
				</div>
		<div  class="margin-t-10" style="text-align:center;padding:5px">
	    	<a href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-small" onclick="submitForm('name','book-editor')"><span class="l-btn-left"><span class="l-btn-text">提交</span></span></a>
...
<script>
		$(function(){
			...
			disableComboTree('protagonists',<c:out value='${fn:length(opus.protagonists)}'></c:out>);
		})
	</script>
